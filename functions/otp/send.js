// Endpoint gửi OTP qua Resend — độc lập, không đụng backend hiện có
  ...
  try {
    ...
    const resendResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: 'Mã xác nhận giao dịch Chuột shop',
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e4e4dc; border-radius: 12px;">
            <h2 style="color: #10110f; margin: 0 0 12px;">Chuột shop — Xác nhận giao dịch</h2>
            <p style="color: #555; font-size: 14px; line-height: 1.6;">Chào bạn,</p>
            <p style="color: #555; font-size: 14px; line-height: 1.6;">Bạn đang thực hiện một giao dịch trên Chuột shop. Vui lòng nhập mã xác nhận dưới đây để hoàn tất:</p>
    ...
    return new Response(JSON.stringify({ ok: true, message: 'Mã xác nhận đã gửi tới email của bạn.' }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' }
    });
  } catch (error) {
